...
################################################################################
# Print a trace message to BASH_XTRACEFD or stderr.
#
...
#
# Outputs:
#   Writes the formatted message to BASH_XTRACEFD or stderr.
...
bake::display::trace() {
  ...
  local xtrace_fd="${BASH_XTRACEFD:-2}"
  ...
  bake::display::format "${xtrace_repeated_character} # ${trace}" >&"${xtrace_fd}"
}
