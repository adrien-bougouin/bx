...
assert_stderr "TERM= bake -q -f '${__TEST_BAKEFILES__}/default_recipe__missing.bakefile'" "$(
	cat <<-EXPECTED
		bake: Nothing to do!
	EXPECTED
)"

assert_stdout "bake -q -f '${__TEST_BAKEFILES__}/default_recipe.bakefile'" "$(
	cat <<-EXPECTED
		Default recipe.
	EXPECTED
)"

assert_stdout "bake -q -f '${__TEST_BAKEFILES__}/default_recipe__subprocess.bakefile'" "$(
	cat <<-EXPECTED
